...
from summary import write_wins_count, read_wins_count
...
#Счётчик побед bot - counter_wins[0], user - counter_wins[1]   
count_points_user = []
count_points_bot = 0
...
def stop(update, context):
    if WINNER == None:
        ...
        if WINNER == -1:
            ...
            write_wins_count(update.effective_user.username, -1)
        elif WINNER == 1:
...
#Функция для вывода числа побед
def summary(update, context):
    data = read_wins_count(update.effective_user.username)
    print(data)
    context.bot.send_message(update.effective_chat.id, f"{data['user']} выйграл {data['user_wins']} раз")
    context.bot.send_message(update.effective_chat.id, f"Бот выйграл {data['bot_wins']} раз")
#
